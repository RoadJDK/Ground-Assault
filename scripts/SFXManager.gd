...
func play_sound(stream: AudioStream, position: Vector2 = Vector2.ZERO, volume_db: float = 0.0, pitch_scale: float = 1.0, overview_dampen: float = 12.0):
	...
	player.max_distance = 4000.0 # Increased range for 2D sounds
...
func play_player_shot(pos: Vector2, extra_db: float = 0.0):
	# Less dampening for player in overview (4dB instead of 12dB)
	play_sound(sfx_player_shot, pos, -5.0 + extra_db, randf_range(0.95, 1.05), 4.0)
